...
class Engine
{
    ...
    public Engine(Field map) {
        int[] snakepos = {map.map.size()/2, map.map.get(0).size()/2};
        if (snakepos == null) {
    ...
    public int run(Field map, Frame_work frame) {
        boolean ended = false;
        while (!go) {}
        map.createApple(frame);
        frame.redraw();
        while (!ended) {
            try{
                waiter.wait(frames);
                ended = !snake.move(map);
                if (!ended) {
                    ...
                    else if (snake.isTile(snake.head.posx, snake.head.posy)) {
                        ended = true;
                    }
                    snake.draw(frame);
                    frame.redraw();
